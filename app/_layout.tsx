...
import { KitProvider } from '../components/kit/KitContext';
...
function RootLayoutInner() {
  ...
  return (
    <>
      {/* Default web document title (a11y: WCAG document-title). Per-screen titles override it;
          set at the root so every route, including nested navigators, has a non-empty title. */}
      <Head>
        <title>{gasConfig.app.name}</title>
      </Head>
      <StatusBar style={resolved === 'dark' ? 'light' : 'dark'} />
      {SentryErrorBoundary ? (
        <SentryErrorBoundary>
          <NavigationGate><Slot /></NavigationGate>
