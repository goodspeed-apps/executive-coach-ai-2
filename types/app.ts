...
// Base types (User, Notification, Bookmark, etc.) live in types/index.ts — do not duplicate.
...
export type MoodValue = string;
export type EnergyValue = string;
...
export interface CoachMessageSection {
  heading: string;
  body: string;
}
...
export interface UnlockedAchievement extends AppAchievement {
...
export interface IntakeEchoResult {
  sections: CoachMessageSection[];
}

export interface BehavioralInsights {
  completion_trend: { week_start: string; completion_rate: number }[];
  trigger_frequencies: { trigger: string; count: number }[];
  avoidance_cycles: { pattern_tag: string; occurrences: number }[];
