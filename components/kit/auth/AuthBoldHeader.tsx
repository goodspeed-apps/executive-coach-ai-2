      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        ...
        >
          ...
          <View style={{ marginBottom: 36 }}>
            {headline.map((line, i) => (
              <Text
                ...
                accessibilityRole={i === 0 ? 'header' : 'none'}
              >
                {line}
              </Text>
            ))}
          ...
          <TouchableOpacity
            ...
            disabled={vm.loading}
            accessibilityRole="button"
            accessibilityLabel={isLogin ? 'Sign in' : isSignup ? 'Sign up' : mode === 'reset' ? 'Send reset link' : 'Set password'}
          ...
          >
            {vm.loading ? (
              ...
              <Text style={{ fontSize: 17, fontWeight: '800', color: colors.textOnPrimary }}>
                {isLogin ? 'Sign in' : isSignup ? 'Create account' : mode === 'reset' ? 'Send reset link' : 'Set password'}
              </Text>
            )}
          </TouchableOpacity>
            ...
            <View style={{ marginTop: 24, gap: 12 }}>
              ...
              {vm.oauthProviders.map((p) => (
                ...
                >
                  ...
                  <Text style={{ fontSize: 15, fontWeight: '600', color: colors.text, textTransform: 'capitalize' }}>{p}</Text>
                </TouchableOpacity>
              ))}
            </View>
          )}
          ...
          <View style={{ marginTop: 28, alignItems: 'center' }}>
            {isLogin && (
              <>
                <Link href="/(auth)/signup" style={{ color: colors.primary, fontSize: 14, fontWeight: '600', marginBottom: 8 }}>
                  Don't have an account? Sign up
                </Link>
                <Link href="/(auth)/reset-password" style={{ color: colors.textSecondary, fontSize: 13 }}>
                  Forgot password?
                </Link>
              </>
            )}
            {isSignup && (
              <Link href="/(auth)/login" style={{ color: colors.primary, fontSize: 14, fontWeight: '600' }}>
