  ...
  return (
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }}>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        <ScrollView
          style={{ flex: 1 }}
          contentContainerStyle={{ flexGrow: 1, paddingHorizontal: 28, paddingTop: 24, paddingBottom: 36 }}
          keyboardShouldPersistTaps="handled"
          showsVerticalScrollIndicator={false}
        >
          {/* Headline */}
          <View style={{ marginBottom: 36, marginTop: 8 }}>
            {headline.map((line, i) => (
              <Text
                key={i}
          ...
          {/* Error */}
          {vm.error ? (
            <Text style={{ color: colors.error, fontSize: 14, marginTop: 8, marginBottom: 4 }} accessibilityRole="alert">
              {vm.error}
            </Text>
          ) : null}
          ...
          {vm.oauthProviders && vm.oauthProviders.length > 0 && (
            <View style={{ marginTop: 28 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 20 }}>
                ...
                <Text style={{ color: colors.textSecondary, fontSize: 13, marginHorizontal: 12 }}>or continue with</Text>
                <View style={{ flex: 1, height: 1, backgroundColor: colors.border }} />
              </View>
              <View style={{ gap: 12 }}>
          ...
          {(isLogin || isSignup) && (
            <View style={{ flexDirection: 'row', justifyContent: 'center', marginTop: 28 }}>
              ...
              <Link href={isLogin ? '/(auth)/signup' : '/(auth)/login'} asChild>
                <TouchableOpacity accessibilityRole="link" accessibilityLabel={isLogin ? 'Sign up' : 'Sign in'}>
