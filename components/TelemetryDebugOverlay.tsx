...
const clientAny = client as unknown as Record<string, unknown>;
  ...
  return (
    <Pressable
      ...
      testID="telemetry-overlay-toggle"
      accessibilityLabel="Toggle telemetry details"
    >
      ...
      {expanded && (
        <ScrollView style={styles.list}>
          {state.recent.length === 0 ? (
            ...
            state.recent.map((ev, i) => (
              <Text key={i} style={styles.eventRow}>
                {ev.timestamp.slice(11, 19)} {ev.type}{ev.screen ? ` (${safeText(ev.screen)})` : ''}
              </Text>
            ))
          )}
        </ScrollView>
