 ...
 *
 * NOTE: each capture's key is written VERBATIM to the buffer (and therefore to
 * the profiles.onboarding_answers jsonb column) with no transformation, the
 * key in the architecture plan's captures[].key IS the stored jsonb key.
 */
...
export async function flushOnboardingAnswers(userId: string): Promise<void> {
  try {
    ...
    // Read the existing column so a partial re-flush merges into prior keys.
    const { data: existing } = await supabase
      .from('profiles')
      .select('onboarding_answers')
      .eq('id', userId)
      .maybeSingle();
    const prior = (existing?.onboarding_answers ?? {}) as OnboardingAnswers;
    const merged = { ...prior, ...answers }; // local buffer wins on collision (newest answer)

    const { error } = await supabase
      .from('profiles')
      .upsert({ id: userId, onboarding_answers: merged }, { onConflict: 'id' });
