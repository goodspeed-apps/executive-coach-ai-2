...
export async function checkMinVersion(): Promise<MinVersionResult> {
  // Web has no installable binary to force-update, so the min-version gate is
  // meaningless there. Returning early also avoids a recurring stream of
  // "Failed to load resource" console.errors (the edge function call, retried
  // up to 3x, fires on every route mount) that pollutes the web render and the
  // render-gate console-error count. Native (iOS/Android) behavior is unchanged.
