...
import { darken, lighten, isLight } from './color';

const c = gasConfig.design.colors;

// Guarantee the dark-mode background is visibly distinct from the light-mode
// background. A generated config sometimes supplies the same hex for both
// `background` and `backgroundDark` (e.g. both '#0D0D0F'), which collapses dark
// mode into light mode, the screen renders wrong and dark mode looks broken.
// When they collide, derive a distinct dark surface from the shared color:
// a light background darkens to near-black; an already-dark one lightens a touch
// so the two modes never read as identical.
function distinctDark(light: string, dark: string): string {
  if (dark.toLowerCase() !== light.toLowerCase()) return dark;
  return isLight(dark) ? darken(dark, 0.92) : lighten(dark, 0.06);
}

const darkBackground = distinctDark(c.background, c.backgroundDark);
