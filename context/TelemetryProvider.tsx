...
export function TelemetryProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      // The ingest endpoint is a cross-origin Goodspeed Studio host. From a web
      // build it cannot be reached without CORS, so every screen_view POST fails
      // with an uncatchable "Failed to load resource" / CORS console.error (one
      // per route navigation) that pollutes the web render and the render-gate
      // console-error count. Telemetry is best-effort analytics, so skip it on
      // web entirely. Native (iOS/Android) telemetry is unchanged.
