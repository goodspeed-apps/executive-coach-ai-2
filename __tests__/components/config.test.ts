...
describe('GAS Config Structure', () => {
  ...
  test('gamification config has required fields', () => {
    ...
    // `elements` is optional at runtime — only `enabled` is read (StreakBadge),
    // and an omitted list behaves as []. So an absent value is valid. A *present*
    // value, however, must be an array — a string/object/number here is a real
    // misconfiguration and still fails.
    expect(g.elements === undefined || Array.isArray(g.elements)).toBe(true);
  });
